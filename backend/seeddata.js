...
async function prepareNamedUserDataAndFiles(name, numOfFiles) {
  try {
    ...
    const createUserResult = await createUser(name, `${name}@designer.com`, '$2b$10$K.0HwpsoPDGaB/atFBmmXOGTw4ceeg33.WrxJx/FeC9.gCyYvIbs6', 2);
    ...
    for (index = 1; index <= numOfFiles; index++) {
      const designTitle = `${name} design ${index}`;
      const designDescription = `${name} design ${index} description text 1 text 2 text 3 text 4 ....`;
      try {
        const uploadResult = await uploadFileToCloudinary(`./${name}_design_files/${name}_${index}.png`);
        const imageURL = uploadResult.imageURL;
        const publicId = uploadResult.publicId;
        console.log('Checking uploadResult before calling createFileData in try block\n', uploadResult);
        try {
          const createFileDataResult = await createFileData(imageURL, publicId, createUserResult.insertId, designTitle, designDescription);
  ...
  return new Promise((resolve, reject) => {
    ...
    pool.getConnection((err, connection) => {
      if (err) {
        ...
        const query = `INSERT INTO file ( cloudinary_file_id, cloudinary_url , 
  ...
  return new Promise((resolve, reject) => {
    ...
    cloudinary.uploader.upload(filePath, {upload_preset: 'upload_to_design'})
        .then((result) => {
          ...
          const data = {imageURL: result.url, publicId: result.public_id, status: 'success'};
          resolve(data);
        }).catch((error) => {
          const message = 'fail';
          reject(error);
...
async function prepareAdminLawrenceData() {
  try {
    const createUserResult = await createUser('Albert', 'Albert@admin.com', '$2b$10$K.0HwpsoPDGaB/atFBmmXOGTw4ceeg33.WrxJx/FeC9.gCyYvIbs6', 1);
    console.log('Inspecting createUserResult variable\n', createUserResult);
  } catch (error) {
