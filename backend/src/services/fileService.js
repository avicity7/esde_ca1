...
module.exports.getFileData = (userId, pageNumber, search) => {
  ...
  if ((search == '') || (search == null)) {
    ...
    designFileDataValues = [userId, limit, offset, userId];
  } else {
    designFileDataQuery = `SELECT file_id,cloudinary_url,design_title,design_description 
    ...
    designFileDataValues = [userId, `%${search}%`, limit, offset, userId, `%${search}%`];
  }
  // --------------------------------------------------------------------
  ...
  return new Promise((resolve, reject) => {
    ...
    pool.getConnection((err, connection) => {
      if (err) {
        ...
        connection.query(designFileDataQuery, designFileDataValues, (err, results) => {
          if (err) {
